...
ee_s0_p = {
    'chinese': '''事件类型"{}"\n\n在这个句子中，事件的触发词是什么？\n请对每个事件类型分别给出句子中的一个动词。\n如果不存在则回答：无\n按照元组形式回复，如 (触发词1, 触发词2, ……)：''',
    'english': '''The event type is "{}"\n\nWhat is the trigger word for the event?\nGive a verb in the sentence for each event type.\nIf not present, answer: none.\nRespond as a tuple, e.g. (trigger 1, trigger 2, ......):'''
}   
...
def chat_ee(inda, chatbot):
    ...
    try:
        ...
        s0p = ee_s0_p[lang].format(rels)
        # print(s0p)

        # 请求chatgpt
        mess.append({"role": "user", "content": s0p})
        text0 = chatbot(mess)
        mess.append({"role": "assistant", "content": text0})
        # print(text0)

        # 正则提取结果
        res0 = re.findall(r'\(.*?\)', text0)
        print(res0)
        if res0!=[]:
            rels0 = [temp[1:-1].split(',') for temp in res0]
            rels0 = list(set([re.sub('[\'"]','', j).strip() for i in rels0 for j in i]))
            #print(rels)
        else: # 说明正则没提取到，可能是单个类型的情况
            text0 = text0.strip().rstrip('.')
            rels0 = [text0]

        print(rels0)
    except Exception as e:
        print(e)
        print('re stage 0 none out or error')
        return ['error-stage0:' + str(e)], mess
...
def create(**args):
    ...
    openai.api_key = "sk-aGJSuwj6bqDibfqNb17N7jMvwHLGQU0zs7o0NanCf02EJa6C"
